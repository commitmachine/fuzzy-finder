...
export default class FuzzyFinderView {
  constructor () {
    ...
    this.selectListView = new SelectListView({
      ...
      didChangeSelection: (item) => {
        if (!this.canceled && item && item != this.selectedItem) {
          this.selectedItem = item
          this.kindaconfirm(item, {searchAllPanes: atom.config.get('fuzzy-finder.searchAllPanes')})
        }
  ...
  kindaconfirm ({filePath}={}, openOptions) {
    for (id in atom.workspace.getTextEditors()){
      if (filePath == atom.workspace.getTextEditors()[id].getBuffer().getPath()){
        if (this.previewEditor){
          this.previewEditor.destroy()
          this.previewEditor = null
          atom.workspace.open(filePath, {activatePane:false, activateItem:true, pending:false})
        }
        return
      }
    }
    atom.workspace.open(filePath, {activatePane:false, activateItem:true, pending:true}).then(result =>
    {
        this.previewEditor = result
    });
  }
